...
import { Button, Col, DatePicker, Divider, Form, Input, Row, Select, Switch, Tabs } from 'antd';
                ...
                {products.map((product: any, i: number) => {
                    return (
                        <Tabs.TabPane key={i} tab={`Order #${i + 1}`}>
                            ...
                            />
                            <Divider plain={true} />
                            <ProductSourcingField
                                name={['sourcingProducts', i]}
                                product={product}
...
const ProductSourcingField: React.FC<{
    name: any[];
    product: any;
    sourcing?: any;
    ...
    isDisabled?: boolean;
    onSerialableChange?: (name: string, value: boolean) => void;
}> = ({ name, product, sourcing, serialable, isDisabled, onSerialableChange }) => {
    const STRUCTURE = {
        deliveryProductSid: {
            dataIndex: 'deliveryProductSid',
            placeholder: 'Product',
                    ...
                    <div>
                            ...
                            return (
                                <div key={field.key} style={{ position: 'relative' }}>
                                    ...
                                    <div style={{ marginBottom: 10 }}>
                                        Sourcing Product #{i + 1}
                                    </div>
                                    <Row gutter={[4, 0]}>
                                        <Col span={8} sm={{ span: 12 }} lg={{ span: 12 }}>
                                            <Form.Item
                                                name={[
                                                    field.name,
                                                    STRUCTURE.sourcingProductSid.dataIndex,
                                                ]}
                                            >
                                                <Select
                                                    options={
                                                        sourcing
                                                            ? sourcing.products.map(
                                                                  (product: any) => ({
                                                                      label: product.name,
                                                                      value: product._sid,
                                                                  })
                                                    ...
                                                    placeholder={
                                                        STRUCTURE.sourcingProductSid.placeholder
                                                    }
                                                />
                                            </Form.Item>
                                        </Col>
                        ...
                        <Form.Item>
                            <Button type='dashed' onClick={() => add()} block disabled={isDisabled}>
                                <PlusOutlined /> Add sourcing product
                            </Button>
                        </Form.Item>
                    </div>
                );
                            ...
                            return (
                                <div key={field.key} style={{ position: 'relative' }}>
                                    ...
                                    <Row gutter={[4, 0]}>
                                        <Col span={8} sm={{ span: 12 }} lg={{ span: 5 }}>
                                            <Form.Item
                                                name={[...name, STRUCTURE.productName.dataIndex]}
