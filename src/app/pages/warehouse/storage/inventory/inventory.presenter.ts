...
export default () => {
    const { loading: isReadLoading, data, cache, get } = useApi('/items');
    const { loading: readOnedIsLoading, data: readOneData, get: readOne } = useApi('/items');
    const { loading: isUpdateLoading, put } = useApi('/items');
